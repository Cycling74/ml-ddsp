...
class ddsp_mfcc_decoder_controls_tilde : public object<ddsp_mfcc_decoder_controls_tilde>, public mc_operator<> {
public:
    ...
    // constructor
    ddsp_mfcc_decoder_controls_tilde(const atoms& args = {}) {
        ...
        if (args.empty()) {
          error("Please specify the input model path as argument.");
            ...
            if (model_is_loaded) { // if loaded correctly
                ...
                auto in_mfccs = std::make_unique<inlet<>>(this, "(multichannelsignal) mfccs");
    ...
    void operator()(audio_bundle input, audio_bundle output) {
        ...
        if (!(m_head % B_SIZE)) { // if it is B_SIZE or B_SIZE * 2
            ...
            m_compute_thread = new std::thread(&ddsp_mfcc_decoder_controls_tilde::thread_perform, this,
...
MIN_EXTERNAL(ddsp_mfcc_decoder_controls_tilde);
